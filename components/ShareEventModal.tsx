...
import { 
  ...
  ActivityIndicator, Alert, StyleSheet, TextInput, ScrollView
} from 'react-native';
...
export function ShareEventModal({ visible, onClose, event, Colors }: ShareEventModalProps) {
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      setFilteredGroups(
        groupChats.filter(chat => chat.username.toLowerCase().includes(query))
      );
    }
  ...
  const copyLinkToClipboard = async () => {
    try {
      ...
      Alert.alert(
        'Enllaç copiat!',
        'L\'enllaç de l\'esdeveniment s\'ha copiat al porta-retalls',
        [{ text: 'D\'acord' }]
      );
    } catch (error) {
