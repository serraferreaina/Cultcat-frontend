...
export function ShareProfileModal({ visible, onClose, profile, Colors }: ShareProfileModalProps) {
  ...
  const [selectedChats, setSelectedChats] = useState<Set<number | string>>(new Set());
  ...
  const loadChats = async () => {
    setLoading(true);
    try {
      const [connections, users, chats, myProfile] = await Promise.all([
        getConnections(),
        getUsers(),
        getGroupChats(),
        getProfile(),
      ...
      const individual = connections
        .filter((c: any) => c.chat_id != null && c.chat_id !== undefined)
        .map((c: any, index: number) => {
          const otherUser = users.find((u: any) => u.id !== myProfile.id && u.id === c.user_id);
          return {
            id: c.chat_id || `individual_${index}`,
      ...
      const groups = chats
        .filter((c: any) => c.chat_id != null && c.participants?.length > 2)
        .map((c: any, index: number) => ({
          id: c.chat_id || `group_${index}`,
          username: c.name || 'Grup sense nom',
          profile: null,
          isGroup: true,
        }))
        .sort((a: ChatItem, b: ChatItem) => a.username.localeCompare(b.username));
  ...
  const toggleChat = (chatId: number | string) => {
  ...
  const copyLinkToClipboard = async () => {
    try {
      const profileUrl = `cultcat://user/${profile.id}`;
      ...
      Alert.alert('Enllaç copiat!', "L'enllaç del perfil s'ha copiat al porta-retalls", [
        { text: "D'acord" },
      ]);
    } catch (error) {
  ...
  const sendToChats = async () => {
    ...
    try {
      const profileData = {
        ...
        description: profile.profile_description?.substring(0, 100) || null,
        userMessage: message.trim() || null,
      };

      const messageContent = JSON.stringify(profileData);
      ...
      Alert.alert(
        'Compartit!',
        `El perfil s'ha compartit a ${selectedChats.size} ${selectedChats.size === 1 ? 'xat' : 'xats'}`,
        [{ text: "D'acord", onPress: onClose }],
      );
    } catch (error) {
      console.error('Error sending messages:', error);
      Alert.alert('Error', "No s'ha pogut compartir el perfil");
    } finally {
      setSending(false);
    }
  };
      ...
      <View style={styles.overlay}>
        <View style={[styles.container, { backgroundColor: Colors.card }]}>
          ...
          <View style={[styles.header, { borderBottomColor: Colors.border }]}>
            ...
            <Text style={[styles.title, { color: Colors.text }]}>Compartir perfil</Text>
          ...
          {/* Profile Preview */}
          <View style={[styles.profilePreview, { backgroundColor: Colors.background }]}>
            <Image source={{ uri: profile.profile_picture }} style={styles.previewAvatar} />
